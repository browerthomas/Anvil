...
- **`agent-completed` event + `/grind-summary` rollup** (closes anvil#75). `/dispatch-slice` gains Step 6: after a sub-agent's completion notification arrives, append one `agent-completed` event to `.anvil/grind-events.jsonl` capturing `{agent, model, branch, total_tokens, tool_uses, duration_ms}`. New helper `av_parse_agent_usage` in `shared/lib.sh` parses three shapes of the `<usage>` block (block form, self-closing inline, JSON body). New writer at `skills/dispatch-slice/scripts/record-agent-completed.sh` — accepts a notification file OR explicit counter flags; soft-fails (exit 0, warning to stderr, no event written) when the usage block is missing / unparseable / `jq` absent / `--slice` absent — older runtimes + manual dispatches must not break. Non-numeric counters are dropped to null at write time (defence-in-depth against a stray label smuggling a string into the event log). New rollup script at `skills/recap/scripts/grind-summary.sh` reads `agent-completed` events, groups by slice (summing multi-agent-per-slice cases — review fix-ups, dispatched fixers), and prints a per-slice + total table with thousands-separator tokens and pretty-printed wall time (Ns / Mm SSs / Hh MMm). The PR column is filled from sibling `slice-pr-opened` events when present. Output always carries the explicit `+ orchestrator: run /cost in Claude Code for parent-side tokens` footer because the runtime tracks parent-side tokens but doesn't expose them as a callable tool — that gap is now named, not hidden. **Token counts only — no dollar conversion**; cloud-model pricing moves too fast for a baked-in rate table to age well. Codex / Hermes CLI calls are not counted (separate subscriptions, not Claude tokens). `cost_usd` stays on the existing `slice-*` events where the runtime already reports it; `agent-completed` deliberately stays rate-table-free. `/grind` step c.5 documents the new event hook between agent return and review; `/recap` Step 5 documents the rollup. Full schema in `docs/observability.md`. Smoke tests at `tests/smoke/dispatch-slice-agent-completed.bats` (9 scenarios — block / inline / JSON usage forms + soft-fail surface + multi-agent accumulation + non-numeric defence) and `tests/smoke/grind-summary.bats` (11 scenarios — empty log / single-slice / multi-agent / multi-slice / PR-less / orchestrator footer / walltime + thousands formatters / --events override / null counter fold).
- **`.anvil/constitution.md` slot + dispatch-prompt-to-disk emission** (speckit-gold S1). New project-local artifact for the strategic frame: north star, inviolable principles, things explicitly out of scope. Loaded by `av_load_constitution` (new helper in `shared/lib.sh`) and prepended as a `## Project constitution` section at the top of every `/dispatch-slice` agent prompt. Empty / missing / whitespace-only / non-UTF-8 → graceful skip (warning to stderr for the UTF-8 case). Two-tier size warning at 2KB and 8KB to keep the slot lean (constitution is paid context on every dispatch). Canonical template at `templates/constitution-template.md` (placeholder sections only — no project-private examples; this is public-facing OSS). `bin/init-anvil-config.sh` scaffolds `.anvil/constitution.md` from the template on first init via `av_resolve_template constitution-template.md` (project overrides under `.anvil/templates/overrides/` honoured by S2's resolver); never overwrites an existing constitution. `/config-bootstrap` deliberately does NOT touch the constitution — auto-synthesis was rejected in adversarial review. The assembled dispatch prompt is now written to `.anvil/dispatched-prompts/<slice-id>.prompt.md` **before** the Agent tool is invoked (hard-fail with `error: cannot write to .anvil/dispatched-prompts/: <reason>` if the directory cannot be created or the file cannot be written). The on-disk emission is load-bearing for grep-testability — without it, the constitution-prepend behaviour is an in-memory `Agent()` argument that bats cannot assert against. `.anvil/dispatched-prompts/` is runtime state and added to both the repo root `.gitignore` and the scaffolded `.anvil/.gitignore`. Smoke tests at `tests/smoke/constitution.bats` (10 scenarios: content / absent / whitespace-only / non-UTF-8 / on-disk emission / no-constitution prompt shape / 2KB warning / 8KB second-tier warning / init scaffold / init non-overwrite).
- **Per-slice checklists in `/pre-merge-gate`** (speckit-gold S3). The slice-manifest YAML schema gains an optional `checklist:` field per slice — a list of `{kind: shell, run, expect, timeout?}` or `{kind: grep, pattern, in, expect, count?}` items that `/pre-merge-gate` enforces at merge time. Two kinds only (`shell` + `grep`); unknown kinds and invalid `expect:` values for grep are refused with a clear stderr message. `shell` items run under a per-item timeout (default 300s, override via `timeout:`); the runner uses a portable shim around `timeout`/`gtimeout`/manual-kill fallback so macOS without coreutils still works. `grep` items support `absent` | `present` (with optional exact `count:` — only meaningful under `present`). Missing `checklist:` field is byte-identical to pre-S3 behaviour — silent absence, no warning. Slice lookup uses `--slice <id>` arg > `.anvil/dispatched-agents.json` exact match (`branch` field == current branch) > hard error before any global gate runs. No branch-name prefix matching — adversarial review enumerated four collision modes (substring match, rebase splits, re-dispatch, branch rename). The `.anvil/dispatched-agents.json` schema is now persisted automatically by `/dispatch-slice` (via `skills/dispatch-slice/scripts/build-prompt.sh`) when invoked with `--plan-path <path>` — `/grind` always sets it; manual dispatchers without `--plan-path` get a row with empty `plan_path` and `/pre-merge-gate` gracefully degrades to legacy global gates only. Ambiguous slice context (same branch maps to multiple JSON entries) hard-fails with the colliding slice ids — no silent first-wins. New helper `av_parse_slice_checklist <plan-path> <slice-id>` in `shared/lib.sh` reuses the existing yq/python3+pyyaml YAML parsing pattern from `skills/spec/scripts/validate.sh`; explicit `jq` precheck (jq is a hard dep) + parser stderr is split from stdout in the caller so YAML errors no longer get swallowed into checklist lines. Numeric `count:` comparison is integer-typed (validated). `templates/plan-template.md` + `templates/plan-folder-template/tasks.md` show the new field with the `count:` example moved under `expect: present` (the only context it applies in). `skills/spec/SKILL.md` Step 2 prompts the operator for 1-3 narrow checklist items per slice. `skills/dispatch-slice/SKILL.md` documents the new `plan_path` field in `.anvil/dispatched-agents.json` (load-bearing — pre-merge-gate uses it to find the plan). Smoke tests at `tests/smoke/per-slice-checklist.bats` (17 scenarios — parser refusal contract + verify.sh slice resolution + checklist execution + timeout + dispatched-agents.json round-trip + non-tautology no-slice-context guard + ambiguous-slice-context refusal).
...
- **`av_resolve_template <name>` 2-layer template resolver** (speckit-gold S2). New helper in `shared/lib.sh` resolves a template through `${PWD}/.anvil/templates/overrides/<name>` (project override) before falling back to `$(av_anvil_root)/templates/<name>` (core default). First hit wins; prints the absolute resolved path on stdout. Refuses path traversal (`..` segments, leading `/`, backslash) before any filesystem access; refuses missing argument; refuses name-not-found-anywhere with stderr message listing both searched paths. Refuses override symlink escape (resolves via `realpath` and asserts the canonical target stays under the expected layer dir). Reuses the existing `av_anvil_root()` helper and the canonical `ANVIL_ROOT` env var — does **not** introduce a new `ANVIL_HOME` variable (that name is overloaded in `bin/`). Skill `/spec` updated to use the resolver via `${ANVIL_ROOT}` (works correctly when sourced from an adopting project's git root). Smoke-tested at `tests/smoke/template-overrides.bats` (override-wins, core-fallback, refusal contract, preset-layer-deferred negative case, symlink-escape guard, foreign-repo sourcing). New documentation at `docs/template-overrides.md` (precedence, why no preset layer yet, worked examples). Preset layer (spec-kit's middle layer) deferred until a real preset use-case appears.
