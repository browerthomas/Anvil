#!/usr/bin/env bash
# resolve-persona.sh — resolve a persona name (bare or category/<name>) to a prompt file
#
# Usage:
#   resolve-persona.sh <name>
#   resolve-persona.sh systems/sre-incident-responder    # explicit category
#   resolve-persona.sh privacy-lawyer                    # bare name; looks across categories
#
# Output:
#   - On match: prints the persona prompt contents to stdout. Status 0.
...
if [ $# -lt 1 ]; then
  echo "usage: resolve-persona.sh <name>" >&2
  exit 1
fi
...
if [[ "$NAME" == */* ]]; then
  TARGET="$PERSONAS_DIR/$NAME.md"
  if [ ! -f "$TARGET" ]; then
    echo "unknown persona: '$NAME'. file not found at $TARGET" >&2
    echo "see skills/persona/SKILL.md 'Available personas' for the catalogue." >&2
    exit 1
  fi
  cat "$TARGET"
  exit 0
...
while IFS= read -r -d '' f; do
  MATCHES+=("$f")
done < <(find "$PERSONAS_DIR" -mindepth 2 -maxdepth 2 -type f -name "$NAME.md" -print0 2>/dev/null)

case ${#MATCHES[@]} in
  0)
    echo "unknown persona: '$NAME'. no match in saas/, systems/, or generic/." >&2
    echo "see skills/persona/SKILL.md 'Available personas' for the catalogue." >&2
    exit 1
    ;;
  1)
    ...
    ;;
  *)
    echo "ambiguous persona: '$NAME' resolves to multiple categories:" >&2
    for m in "${MATCHES[@]}"; do
